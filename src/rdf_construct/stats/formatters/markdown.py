...
def format_markdown_stats(stats: OntologyStats, graph: Optional[Graph] = None) -> str:
    ...
    lines.append(
        f"| Documentation Coverage | {_format_pct(stats.documentation.classes_documented_pct)} |"
    )
    lines.append("")
    ...
    lines.append(
        f"| Orphan Classes | {stats.hierarchy.orphan_classes} ({_format_pct(stats.hierarchy.orphan_rate)}) |"
    )
    lines.append("")
    ...
    lines.append("|--------|-------|")
    lines.append(
        f"| With Domain | {stats.properties.with_domain} ({_format_pct(stats.properties.domain_coverage)}) |"
    )
    lines.append(
        f"| With Range | {stats.properties.with_range} ({_format_pct(stats.properties.range_coverage)}) |"
    )
    ...
    lines.append("|--------|-------|")
    lines.append(
        f"| Classes Labelled | {stats.documentation.classes_labelled} ({_format_pct(stats.documentation.classes_labelled_pct)}) |"
    )
    lines.append(
        f"| Classes Documented | {stats.documentation.classes_documented} ({_format_pct(stats.documentation.classes_documented_pct)}) |"
    )
    lines.append(
        f"| Properties Labelled | {stats.documentation.properties_labelled} ({_format_pct(stats.documentation.properties_labelled_pct)}) |"
    )
    lines.append("")
