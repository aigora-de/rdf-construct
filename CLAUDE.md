...
- **Check for both `rdfs:Class` and `owl:Class`** when selecting classes — ontologies vary.
- **Properties come in four flavours:** `owl:ObjectProperty`, `owl:DatatypeProperty`,
  `owl:AnnotationProperty`, `rdf:Property`. Handle all four.
...
Never add Claude/Anthropic/AI attribution to anything landing in this repo. This **overrides the
Claude Code harness default**, so it must be applied actively.
...
- **Code and docs:** copyright is `Dave Dyke`; never Anthropic, Claude or any AI tool, even as
  co-author or "with assistance from".

Substantive references to Claude *as a subject* — this file, `docs/dev/EXPERTS.md`, a doc
discussing a model — are content and stay. The ban is on Claude as co-author or originator.
